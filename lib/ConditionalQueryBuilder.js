...
const expression = require('./ExpressionBuilder');
...
class ConditionalQueryBuilder extends QueryBuilder {
    ...
    in(array) {
        ...
        this.addCondition(`IN ${expression_array.join(', ')}`);
        return this;
    }
    ...
    where(attribute, condition, value) {
        ...
        this[condition](this.addExpressionName(attribute), this.addExpressionValue(value));
        return this;
    }

    createConditionList(attributes, condition) {
        attributes = !Array.isArray(attributes) ? [attributes] : attributes;
        const conditionList = attributes.reduce((list, attribute) => {
    ...
    contains(attribute, substr) {
        this.addCondition(`contains(${attribute}, ${substr})`);
    }
    ...
    query(key, op, condition) {
        ...
        const params = this.buildParams({}, 'ALL_NEW');
    ...
    update(Key, updateParams) {
        ...
        return this.doc('update', this.buildParams({ Key }, 'UPD'))
    ...
    buildParams( params = {}, toReturn = 'OLD' ) {
        ...
        this.params = this.resetExpressionParams();
    ...
    buildUpdateExpression() {
        ...
        const type = expression.indexOf('=') >= 0
            ? UPDATE_TYPES.SET
            : UPDATE_TYPES.REMOVE;
        this.params.UpdateExpression = `${type} ${expression}`;
    }
    ...
    buildConditionExpression() {
        ...
        if ( !(conditions.length && keyConditions.length) ) {
            return;
        }
    ...
    batchGet(batchParams) {
        ...
        Object.keys(batchParams).forEach(tableName => {
            ...
            const currentTable = params.RequestItems[tableName];
            const providedKeys = batchParams[tableName].Keys;
            Object.keys(providedKeys).forEach((key) => {
                providedKeys[key].forEach(keyValue =>
                    currentTable.Keys.push({ [key]: keyValue })
                );
            });
        });
